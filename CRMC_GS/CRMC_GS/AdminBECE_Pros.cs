...
namespace CRMC_GS
{
    public partial class AdminBECE_Pros : Form
    {
        ...
        public AdminBECE_Pros()
        {
            InitializeComponent();
        }

        private void AdminBECE_Pros_Load(object sender, EventArgs e)
        {
            DisplayRecord();
        }
        private void DisplayRecord()
        {
            SqlCommand cmd = new SqlCommand("Select *From BECE_Pros", con);
        ...
        public void Browse() {
            ...
            con.Open();
            SqlDataAdapter adapt = new SqlDataAdapter("select * from BECE_Pros where StudentsYear like '" + cbYearLevel.Text + "%' AND Semester like '" + cbsemester.Text + "%'", con);
        ...
        private void txtSearch_KeyPress(object sender, KeyPressEventArgs e)
        {
            con = new SqlConnection(connectionstring);
            con.Open();
            SqlDataAdapter adapt = new SqlDataAdapter("select * from BECE_Pros where Title like '" + txtSearch.Text + "%'", con);
            DataTable dt = new DataTable();
            adapt.Fill(dt);
            dataGridView1.DataSource = dt;
            con.Close();
        ...
        private void AdminBECE_Pros_FormClosing(object sender, FormClosingEventArgs e)
        {
