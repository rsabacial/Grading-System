...
namespace CRMC_GS
{
    public partial class BSE_ENGLISH_Pros : Form
    {
        ...
        public BSE_ENGLISH_Pros()
        {
            InitializeComponent();
        }

        private void BSE_ENGLISH_Pros_Load(object sender, EventArgs e)
        {
            DisplayRecord();
        }
        private void DisplayRecord()
        {
            SqlCommand cmd = new SqlCommand("Select *From BSEENGLISH_Pros", con);
        ...
        public void browse() {
            ...
            con.Open();
            SqlDataAdapter adapt = new SqlDataAdapter("select * from BSEENGLISH_Pros where StudentsYear like '" + cbYearLevel.Text + "%' AND Semester like '" + cbsemester.Text + "%'", con);
        ...
        private void txtSearch_KeyPress(object sender, KeyPressEventArgs e)
        {
            con = new SqlConnection(connectionstring);
            con.Open();
            SqlDataAdapter adapt = new SqlDataAdapter("select * from BSA_Pros where Title like '" + txtSearch.Text + "%'", con);
            DataTable dt = new DataTable();
            adapt.Fill(dt);
            dataGridView1.DataSource = dt;
            con.Close();
        ...
        private void BSE_ENGLISH_Pros_FormClosing(object sender, FormClosingEventArgs e)
        {
